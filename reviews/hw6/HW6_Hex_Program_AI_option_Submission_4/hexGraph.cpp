...
inline bool hexGraph::hasWon(const hexBoard* board, const Space color) const {
    if (color == BLACK) return isConnectedDFS(board, pseudo_bottom, pseudo_top, BLACK);
    if (color == RED) return isConnectedDFS(board, pseudo_left, pseudo_right, RED);
    assert(false); // Color should always be either black or white.
    return false;
}

Space hexGraph::checkWinner(const hexBoard* board) const {
    if (hasWon(board, RED)) return RED;
    if (hasWon(board, BLACK)) return BLACK;
    return EMPTY;
}

inline Space hexGraph::oppositeColor(Space color) {
    if (color == BLACK) return RED;
    else if (color == RED) return BLACK;
    else assert(false);
}
...
pair<int, int> hexGraph::getBestAIMoveWeight(const hexBoard &board, const int &iterations, const int &plies, int threads, const Space 
&this_player) const {
	...
	thread thr = thread(&hexGraph::getAIMoveWeights, this, ref(board), ref(iters_per_thread), ref(plies), ref(this_player), 
ref(unused_spaces), ref(*(thread_move_weights[i])));
...
void hexGraph::getAIMoveWeights(const hexBoard &board, const int &iterations, const int &plies, const Space &this_player, const vector<int> &moves, vector<int> &result) 
const {
...
int hexGraph::getMonteCarloWeight(const hexBoard &board, const int &iterations, const Space &this_player, const int &move) const {
    ...
    for (int i = 0; i < carlo_board_size; i++) {
        if (carlo_board.getSpace(i) == EMPTY) {
            unused_spaces_count++;
            unused_spaces.push_back(i);
        }
    }
